...
export function resolvePlateAppearance(input: {
  ...
  const readHitBonus = perfectTarget
    ...
    : nearTarget && covered
      ...
      : 0;
  const hitChance = clamp(0.305 + input.batter.a / 350 + (input.swing === "contact" ? 0.02 : 0) + hitQuality + locationHitBonus + readHitBonus - input.pitcher.s / 650 - input.pitcher.v / 1280 + (mistake ? 0.05 : 0));
  const roll = random();
