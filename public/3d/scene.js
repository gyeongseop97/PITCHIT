...
import {playerProfile,PITCH_FORMS,BAT_FORMS} from './player-profiles.js';
...
function animatePitcher(t){
 ...
 pitcher.hip.position.set(-.025*lift,.80-.10*drive+.10*recover,.29*drive*(1-recover));pitcher.hip.rotation.set(0,-.2*lift+.22*drive*(1-recover),0);pitcher.torso.rotation.set(.05+.25*drive*(1-recover),-pitchForm.coil*lift+.22*drive*(1-recover),-.04*drive);
 ...
 const ready=new THREE.Vector3(-.01,1.26,.24),cocked=new THREE.Vector3(-.47,1.68,-.13),released=new THREE.Vector3(pitchForm.reach,pitchForm.slot,.72),follow=new THREE.Vector3(.17,.98,.58);
...
function animateBatter(dt){
 ...
 const crouch=Math.max(0,1-y)*.5*extension+batForm.crouch*(1-extension);
 batter.hip.position.set(0,.77-crouch,.025*turn+.075*extension);batter.hip.rotation.y=turn*.38;batter.torso.rotation.set(.08+crouch*.4,-load+turn*.60,0);
 batter.head.rotation.set(.025,state.phase==='flight'?lerp(1.1,.12,clamp(state.flight/state.duration,0,1)):live?.12:1.1,0);
 plantFoot(batter,0,new THREE.Vector3(batForm.width,.08,.035));plantFoot(batter,1,new THREE.Vector3(-batForm.width+.01,.08+turn*.04,.015));
...
function sync(next){const previousRole=model.defending,previousPitch=pitchSign,previousBat=batSign;
 ...
 if(next?.pending||state.phase!=='ready'){pendingModel=next||{};model={...model,locked:true,shop:next?.shop??model.shop};}
