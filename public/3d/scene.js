...
function animateBatter(dt){
 ...
 const turn=live?ease(age/.28)*(1-ease((age-.57)/.38)):0,load=state.phase==='windup'?Math.sin(state.elapsed/TIMING.windup*Math.PI)*.14:0;
 ...
 const crouch=Math.max(0,1-y)*.5*extension;
 ...
 if(live){
  ...
  grip.lerp(new THREE.Vector3(target.x/batSign-.65,y,0),u).lerp(new THREE.Vector3(-.72,1.40,-.15),v).lerp(new THREE.Vector3(-.62,1.26,.23),recover);
...
let runRoute=[],runDuration=0,runnerPlans=[],choreography=null,battedFlight=null;
...
function animateRunner(t){for(const r of runnerPlans){const p=r.from===0?batterRunner:runners[r.from-1];if(t<r.delay){if(r.from>0){p.root.visible=true;p.root.position.fromArray(BASES[r.from]);runPose(p,0,false);}continue;}
 if(r.from===0&&!r.takeoff){
  r.takeoff=batter.bodyMesh.skeleton.bones.map(b=>({position:b.position.clone(),rotation:b.quaternion.clone()}));
  r.start=batter.root.getWorldPosition(new THREE.Vector3());p.root.scale.x=batSign;p.badge.scale.x=batSign;
 }
 const stopped=Number.isFinite(r.stopAt)&&t>=r.stopAt,elapsed=Math.min(t,r.stopAt??Infinity)-r.delay,distance=r.retreat||r.returnAfterForce?1:Math.max(0,r.to-r.from),duration=r.duration,acceleration=.3,travel=elapsed<acceleration?elapsed*elapsed/(2*acceleration):elapsed-acceleration/2,progress=clamp(travel/(duration-acceleration/2),0,1),walk=r.walk;
 p.root.visible=!(r.out&&Number.isFinite(r.outAt)&&t>=r.outAt+.18)&&!(progress>=1&&(r.out||r.to===4));if(!p.root.visible)continue;
 if(r.retreat||r.returnAfterForce){const v=Math.sin(progress*Math.PI)*.18,from=r.start||new THREE.Vector3(...BASES[r.from]),to=new THREE.Vector3(...BASES[r.attemptTo||1]);p.root.position.lerpVectors(from,to,v);const direction=to.clone().sub(from).multiplyScalar(progress<.5?1:-1);p.root.rotation.y=Math.atan2(direction.x,direction.z);}
 else if(distance){const segment=progress*distance,index=Math.min(distance-1,Math.floor(segment)),u=clamp(segment-index,0,1),from=r.from===0&&index===0?r.start:new THREE.Vector3(...BASES[r.from+index]),to=new THREE.Vector3(...BASES[r.from+index+1]);p.root.position.lerpVectors(from,to,u);if(index<distance-1){const rounding=Math.sin(u*Math.PI)*.8;p.root.position.x+=rounding;}const direction=to.clone().sub(from);p.root.rotation.y=Math.atan2(direction.x,direction.z);}
 else p.root.position.fromArray(BASES[r.from]);
 runPose(p,t*(walk?.6:1),progress<1&&distance>0&&!stopped,r.slide&&progress>.89&&progress<1);
...
function startRunners(){batterRunner.root.visible=false;const throws=defense.throwTimes;
 runnerPlans=choreography.runs.map(r=>{const outAt=r.out&&choreography.ground?throws.find(l=>l.base===r.to)?.time:undefined;const duration=r.returnAfterForce?Math.max(1,2*((throws[0]?.time||choreography.catchAt)-r.delay)+.4):r.retreat?choreography.catchAt+.5:r.walk?5:choreography.home?Math.max(1,r.to-r.from)*2.4:Math.max(1,r.to-r.from)*(r.from===0&&r.to===1?3.5:2.4);return {...r,outAt,stopAt:choreography.inningEnded&&!r.out?throws.at(-1)?.time:undefined,duration:outAt!=null?Math.max(duration,outAt+.12-r.delay):duration}});runRoute=runnerPlans.some(r=>r.from===0&&r.to>0)?BASES.slice(1,(runnerPlans.find(r=>r.from===0)?.to||0)+1):[];runDuration=Math.max(0,...runnerPlans.map(r=>Math.min(r.stopAt??Infinity,r.outAt!=null?r.outAt+.18:Infinity,r.delay+r.duration)));runners.forEach((p,i)=>{p.root.visible=Boolean(currentPlay.basesBefore?.[i]);p.root.position.fromArray(BASES[i+1])});}
