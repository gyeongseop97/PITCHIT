...
function animatePitcher(t){
 const w=state.phase==='windup'?clamp(t/(TIMING.windup*pitchForm.tempo),0,1):state.phase==='flight'?1+state.flight/.7:state.phase==='result'?1.7+state.resultAge:0;
...
function animateBatter(dt){
 ...
 if(live){
  ...
  grip.lerp(new THREE.Vector3(target.x/batSign-.65,y,0),u).lerp(new THREE.Vector3(-.72,1.40,-.15),v).lerp(new THREE.Vector3(-.62,batForm.gripY,batForm.gripZ),recover);
  const angle=lerp(1.75,0,u)-v*2;direction.set(Math.cos(angle),lerp(.7,0,u)+v*.7,Math.sin(angle)).lerp(new THREE.Vector3(-.18,.91,.36),recover);
 }
...
function animateRunner(t){for(const r of runnerPlans){
 ...
 const stopped=Number.isFinite(r.stopAt)&&t>=r.stopAt,elapsed=Math.max(0,Math.min(t,r.stopAt??Infinity)-r.delay),distance=r.retreat||r.returnAfterForce?1:Math.max(0,r.to-r.from),duration=r.duration,acceleration=.3,travel=elapsed<acceleration?elapsed*elapsed/(2*acceleration):elapsed-acceleration/2,progress=clamp(travel/(duration-acceleration/2),0,1),walk=r.walk;
 ...
 const sliding=r.slide&&progress>.88&&progress<1&&!stopped;
 runPose(p,t*(walk?.6:1),(progress<1||r.runThrough&&elapsed<duration+.65)&&distance>0&&!stopped,sliding);
 if(sliding){p.hip.position.y=lerp(.78,.25,clamp((progress-.88)/.08,0,1));p.torso.rotation.x=-.48;}
...
function startRunners(){batterRunner.root.visible=false;const throws=defense.throwTimes;
 runnerPlans=choreography.runs.map(r=>{
  const outAt=r.out?(choreography.fly?choreography.catchAt:throws.find(l=>l.base===r.to)?.time):undefined;
  const speed=r.speed&&r.speed!==1?r.speed:55,pace=clamp(2.65-(speed-55)/110,2.3,3.1);
  let duration=r.returnAfterForce?Math.max(1,2*((throws[0]?.time||choreography.catchAt)-r.delay)+.4):r.retreat?choreography.catchAt+.3:r.walk?4.8:choreography.home?Math.max(1,r.to-r.from)*2.15:Math.max(1,r.to-r.from)*(r.from===0&&r.to===1?3.25:pace);
  // Preserve running pace on easy outs; a runner must not beat an out throw.
  // Safe runners beat the receiving throw.
  const receiving=throws.find(l=>l.base===r.to)?.time;
  if(outAt!=null&&!r.retreat&&!choreography.fly)duration=Math.max(duration,outAt+.14-r.delay);
  else if(receiving!=null&&r.to>r.from&&!r.retreat&&!choreography.fly&&!choreography.home)duration=Math.min(duration,Math.max(.65,receiving-.18-r.delay));
  const runThrough=r.from===0&&r.to===1&&!r.out&&!r.walk;
  return {...r,outAt,runThrough,stopAt:choreography.inningEnded&&!r.out?(choreography.fly?choreography.catchAt:throws.at(-1)?.time):undefined,duration};
 });
...
function revealResult(){if(state.revealed)return;state.revealed=true;
 ...
 lastCommentary=description;onStatus(null,{history:[...pitchHistory],reveal:true,commentary:lastCommentary});
}
function scheduleCalls(){
 ...
 if(shot.ground&&!shot.hit)for(const move of runnerPlans.filter(r=>r.out)){const at=throws.find(l=>l.base===move.to)?.time;if(at!=null)callEvents.push({at,title:(move.to===4?'홈':move.to+'루')+'에서 아웃!'});}
 ...
 const decisive=shot.foul?callEvents[0].at:Math.max(shot.fly?shot.catchAt:shot.ground&&!shot.hit?(throws.at(-1)?.time||shot.catchAt):shot.home?(callEvents[0]?.at||shot.catchAt):0,...arrivals.map(e=>e.at));
