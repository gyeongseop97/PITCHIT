...
function animateRunner(t){for(const r of runnerPlans){const p=r.from===0?batterRunner:runners[r.from-1];if(t<r.delay){if(r.from>0){p.root.visible=true;p.root.position.fromArray(BASES[r.from]);runPose(p,0,false);}continue;}
 ...
 const elapsed=t-r.delay,distance=r.retreat?1:Math.max(0,r.to-r.from),duration=r.duration,acceleration=.3,travel=elapsed<acceleration?elapsed*elapsed/(2*acceleration):elapsed-acceleration/2,progress=clamp(travel/(duration-acceleration/2),0,1),walk=r.walk;
 p.root.visible=!(progress>=1&&(r.out||r.to===4));if(!p.root.visible)continue;
 if(r.retreat){const v=Math.sin(progress*Math.PI)*.18;p.root.position.lerpVectors(r.start,new THREE.Vector3(...BASES[1]),v);p.root.rotation.y=progress<.5?Math.PI*.75:-Math.PI*.25;}
