...
export function createDefense({THREE,player,createGlove,scene,pitcher}) {
 ...
 function begin(play,start){reset();const shot=play.choreography||planPlay(play);if(!shot.contact||shot.home||shot.foul)return false;
  ...
  let source=fielder,cursor=shot.catchAt;const legs=shot.throws.map(base=>{let receiver=base===1?fielders[0]:base===3?fielders[3]:fielders[1]===source?fielders[2]:fielders[1];if(base===4&&catcher)receiver=catcher;if(receiver===source)receiver=base===1?pitcher:fielders[2];const position=new THREE.Vector3(...BASES[base]);const gather=shot.kind==='double_play'?TIMING.doublePlayTransfer:TIMING.transfer,duration=Math.max(.42,(base===1?29:source.home.distanceTo(position))/TIMING.throwSpeed);const leg={source,receiver,base,position,start:cursor,release:cursor+gather,end:cursor+gather+duration};source=receiver;cursor=leg.end;return leg});
  plan={...shot,fielder,from,end,catchPoint,start:start.clone(),style,legs,duration:cursor+TIMING.settle};stage='chase';return true;
 }
 ...
 function cameraFrame(ball,aspect){const center=ball.clone().multiplyScalar(.45).addScaledVector(plan.fielder.root.position,.25).addScaledVector(new THREE.Vector3(5,0,-22),.3);center.y=Math.max(1.2,center.y);const distance=(plan.ground?37:53)/Math.max(.65,Math.min(1,aspect));return {position:center.clone().add(new THREE.Vector3(.1,.65,.85).multiplyScalar(distance)),target:center};}
