...
export function createDefense({THREE,player,sphere,scene,pitcher}) {
 ...
 function update(t,ball){if(!plan)return false;const p=plan,f=p.fielder;
  ...
  if(t<p.catchAt){stage='chase';const u=clamp(t/p.catchAt),run=clamp(u*1.15);f.root.position.lerpVectors(p.from,p.end,run);ready(f);face(f,run<1?p.end:p.start);const stride=Math.sin(t*15)*.6*(run<1?1:0);f.legs[0].pivot.rotation.x=stride;f.legs[1].pivot.rotation.x=-stride;if(u>.86)pose(f,p.ground||p.hit,p.style);
   if(p.hit&&!p.infieldHit){const landing=.63;if(u<landing){ball.position.lerpVectors(p.start,p.catchPoint,u);ball.position.y=.085+(p.start.y-.085)*(1-u/landing)+Math.sin(Math.PI*u/landing)*p.apex;}else{ball.position.lerpVectors(p.start,p.catchPoint,u);ball.position.y=.085+Math.abs(Math.sin((u-landing)*Math.PI*9))*.5*(1-u);if(u>.9)ball.position.lerp(p.catchPoint,(u-.9)/.1);}}
   else{ball.position.lerpVectors(p.start,p.catchPoint,u);ball.position.y+=p.ground?Math.abs(Math.sin(u*Math.PI*(3+p.variant*2)))*.38*(1-u):Math.sin(Math.PI*u)*p.apex;}
  }else{
 ...
 reset();return {settle(){pitcher.root.position.copy(pitcher.home);fielders.forEach(p=>{p.root.position.copy(p.home);p.root.rotation.set(0,0,0);ready(p)})},setCatcher(p){catcher=p;catcher.name='포수'},setUniform(color){fielders.forEach(p=>p.setUniform(color))},begin,update,reset,cameraFrame,get active(){return Boolean(plan)},get stage(){return stage},get duration(){return plan?.duration||0},get label(){if(!plan)return '';const leg=plan.legs[throwIndex];return stage==='chase'?`${plan.fielder.name} 타구 처리`:stage==='gather'?`${leg?.source.name||plan.fielder.name} 송구 준비`:stage==='throw'?`${leg.source.name} → ${leg.base===4?'홈':leg.base+'루'} 송구`:'포구 완료'},snapshot(){return {stage,kind:plan?.kind,style:plan?.style,angle:plan?.angle,throwIndex,throws:plan?.throws||[],fielders:fielders.map(p=>({name:p.name,position:p.root.position.toArray()})),fielder:plan?.fielder.name,receiver:plan?.legs[throwIndex]?.receiver.name}}};
}
