...
randomizeHands();
let teamColors=['#ece1bb','#9d4336'];
function seedTeams(seed){let h=0;for(const c of String(seed))h=Math.imul(h,31)+c.charCodeAt(0)|0;const pairs=[['#f2dfb3','#254b80'],['#78c6ba','#913c53'],['#e8b84b','#524876'],['#e8ddd5','#1e685c'],['#e58e68','#345578'],['#8db4df','#843a30']];teamColors=pairs[(h>>>0)%pairs.length];}
...
function resetCatcher(){catcher.root.position.set(0,0,1.45);catcher.root.rotation.y=Math.PI;catcher.hip.position.y=.5;catcher.torso.rotation.set(0,0,0);catcher.legs.forEach((leg,i)=>{leg.pivot.rotation.set(-1,0,i?-.65:.65);leg.knee.rotation.x=1.75});catcher.left.pivot.rotation.set(-1,0,0);catcher.left.elbow.rotation.x=-.5;}
