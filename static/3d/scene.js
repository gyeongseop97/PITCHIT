...
function label(text,w,h,bg,fg){const c=document.createElement('canvas');c.width=1024;c.height=256;const cctx=c.getContext('2d');cctx.fillStyle=bg;cctx.fillRect(0,0,1024,256);cctx.fillStyle=fg;cctx.font='900 120px Arial';cctx.textAlign='center';cctx.textBaseline='middle';cctx.fillText(text,512,135);const texture=new THREE.CanvasTexture(c);texture.colorSpace=THREE.SRGBColorSpace;return mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide}));}
...
function player(color,x,z,number){
 ...
 const badge=label(number,.19,.22,color,'#f9f5e7');torso.add(badge);badge.position.set(.08,.3,.158);
 ...
 return {root,hip,torso,left,right,legs,badge};
}
...
const defense=createDefense({THREE,player,sphere,scene});
...
let runRoute=[],runDuration=0;
function animateRunner(t){if(!runRoute.length||t<.35)return;battingRig.scale.x=1;const start=new THREE.Vector3(-.82*batSign,0,.05),legTime=runDuration/runRoute.length,progress=Math.min(runRoute.length,t/legTime),index=Math.min(runRoute.length-1,Math.floor(progress)),from=index?runRoute[index-1]:start,to=runRoute[index];batter.root.position.lerpVectors(from,to,clamp(progress-index,0,1));const delta=to.clone().sub(from);batter.root.rotation.y=Math.atan2(delta.x,delta.z);batter.hip.rotation.y=0;batter.hip.position.y=.86+Math.abs(Math.sin(t*16))*.035;batter.torso.rotation.set(.16,0,0);const stride=t>=runDuration?0:Math.sin(t*16)*.75;batter.legs.forEach((leg,i)=>{leg.pivot.rotation.set(i?stride:-stride,0,0);leg.knee.rotation.x=Math.max(0,i?-stride:stride)*.9});batter.left.pivot.rotation.set(stride,0,-.12);batter.right.pivot.rotation.set(-stride,0,.12);batter.left.elbow.rotation.set(-1,0,0);batter.right.elbow.rotation.set(-1,0,0);}
...
function sync(next){model=next||{};if(!model.pending)runners.forEach((runner,i)=>runner.root.visible=Boolean(model.bases?.[i]));targetMarker.visible=Number.isInteger(model.pick)&&state.phase!=='flight';if(targetMarker.visible)targetMarker.position.copy(cellPoint(model.pick));
...
function isContact(){return ['single','double','triple','homerun','groundout','infield_flyout','outfield_flyout','foul'].includes(state.outcome)||/안타|루타|홈런|땅볼|뜬공|파울/.test(currentPlay.text||'')}
function resolveVisual(){state.phase='result';state.resultAge=0;const out=state.outcome,text=currentPlay.text||'';state.contact=isContact();
 if(state.contact){const home=out==='homerun'||/홈런/.test(text),ground=out==='groundout'||/땅볼/.test(text),foul=out==='foul'||/파울/.test(text),fly=/flyout/.test(out)||/뜬공/.test(text);velocity.set(foul?23:6,home?24:ground?2:fly?20:9,home?-33:ground?-17:fly?-14:-25);battedStart.copy(ball.position);trail.visible=!reduced;const effect=model.shop?.equippedEffect||'classic';contactRing.material.color.set({classic:'#f4edd3',night:'#ff9950',retro:'#d3ad73',neon:'#8cddff'}[effect]||'#f4edd3');contactRing.position.copy(ball.position);contactRing.material.opacity=.85;}else{trail.visible=false;}
 if(state.contact)defense.begin(currentPlay,battedStart);
 if(state.contact&&!/파울/.test(text)&&out!=='foul'){const count=out==='homerun'||/홈런/.test(text)?4:out==='triple'||/3루타/.test(text)?3:out==='double'||/2루타/.test(text)?2:1;runRoute=[[19.4,0,-19.4],[0,0,-38.8],[-19.4,0,-19.4],[0,0,0]].slice(0,count).map(p=>new THREE.Vector3(...p));runDuration=count===1?3.3:count*1.8;}
 ...
 const description=describePlay(currentPlay);pitchHistory.unshift({...lastLocation,pitch:description.pitch,result:description.title});pitchHistory.splice(6);onStatus(null,{history:[...pitchHistory],reveal:true});
 runners.forEach((runner,i)=>runner.root.visible=Boolean(model.bases?.[i]));lastCommentary=describePlay(currentPlay);onStatus(null,{commentary:lastCommentary});
}
...
function update(dt){state.elapsed+=dt;animatePitcher(state.elapsed);animateBatter(dt);
 ...
 else if(state.phase==='result'){state.resultAge+=dt;const t=state.resultAge;contactRing.material.opacity=Math.max(0,.85-t*3);contactRing.scale.setScalar(1+t*3);if(state.contact&&defense.active){defense.update(t,ball);trail.visible=defense.stage==='throw'||defense.stage==='chase';trailAt(ball.position);}else if(state.contact){ball.position.copy(battedStart).addScaledVector(velocity,t);ball.position.y-=4.905*t*t;ball.rotation.x+=dt*22;trailAt(ball.position);if(ball.position.y<.085){ball.position.y=.085;trail.visible=false;}}else{ball.position.copy(gloveTarget);ball.visible=t<.45;}const end=defense.active?defense.duration:state.contact?Math.min(5.5,(velocity.y+Math.sqrt(velocity.y*velocity.y+19.62*battedStart.y))/9.81+.4):1.1;if(t>Math.max(end,runDuration+.35)){state.phase='ready';ball.visible=false;trail.visible=false;following=false;if(!followSuppressed)setCamera('zone',false);sync(model);}}
 ...
 camera.lookAt(lookCurrent);camera.updateMatrixWorld();batter.root.visible=view!=='zone'||state.phase!=='ready';batPivot.visible=batter.root.visible&&!(runRoute.length&&state.phase==='result'&&state.resultAge>.35);catcher.root.visible=!['batter','zone'].includes(view);visibleMitt.visible=!catcher.root.visible&&(state.phase==='catch'||(state.phase==='result'&&!state.contact));if(visibleMitt.visible){catcherGlove.getWorldPosition(visibleMitt.position);catcherGlove.getWorldQuaternion(visibleMitt.quaternion);}zone.visible=!following;targetMarker.visible=zone.visible&&Number.isInteger(model.pick)&&state.phase==='ready';locationMarker.visible=zone.visible&&Boolean(lastLocation&&!lastLocation.outside&&Number.isInteger(lastLocation.cell));paintZone();
}
...
return {sync,play,setRoomSeed(seed){randomizeHands(seed)},reset(){state.phase='ready';state.visualKey='';state.plays=0;currentPlay=null;lastLocation=null;lastCommentary=null;runRoute=[];runDuration=0;randomizeHands();following=false;state.swingAge=10;pitchHistory.length=0;defense.reset();setCamera('zone',false);onStatus(null,{history:[],commentary:{title:'어떤 코스로 승부할까요?',detail:'가상 스트라이크존에서 코스를 선택하세요.',pitch:''}})},setCamera,setFollow(value){followEnabled=Boolean(value);if(!followEnabled&&following)setCamera('zone',false)},setActive(value){active=value;previous=performance.now()},snapshot(){return{phase:state.phase,key:state.visualKey,plays:state.plays,outcome:state.outcome,actualCell:currentPlay?.actualCell,ball:ball.position.toArray(),bases:[...(model.bases||[])],pick:model.pick,moundDistance:18.44,camera:view,following,cameraPosition:camera.position.toArray(),cameraTarget:lookCurrent.toArray(),commentary:lastCommentary,history:[...pitchHistory],location:lastLocation,rangeCells:[...(model.rangeCells||[])],rangeWeights:{...model.rangeWeights},handedness:{pitch:pitchSign===1?'R':'L',bat:batSign===1?'R':'L'},bodyYaw:batter.root.rotation.y,batterPosition:batter.root.position.toArray(),running:state.phase==='result'&&runRoute.length>0&&state.resultAge>.35,runBases:runRoute.length,contact:state.contact,effectOpacity:contactRing.material.opacity,glove:gloveTarget.toArray(),swingAge:state.swingAge,batPosition:batPivot.position.toArray(),batVisible:batPivot.visible,defense:defense.snapshot(),zoneCells:zoneCells.map(tile=>{const p=tile.getWorldPosition(new THREE.Vector3()).project(camera);return{cell:tile.userData.cell,x:(p.x+1)/2,y:(1-p.y)/2,visible:zone.visible&&p.z>-1&&p.z<1}})}},dispose(){disposed=true;cancelAnimationFrame(frameId);observer.disconnect();for(const [type,fn] of [['pointerdown',down],['pointerup',up],['pointermove',move],['pointerleave',leave],['pointercancel',leave],['keydown',keydown]])renderer.domElement.removeEventListener(type,fn);scene.traverse(object=>{object.geometry?.dispose();if(object.material){for(const material of Array.isArray(object.material)?object.material:[object.material]){material.map?.dispose();material.dispose()}}});renderer.dispose();renderer.domElement.remove()}};
}
