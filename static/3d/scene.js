...
import {playerProfile,PITCH_FORMS,BAT_FORMS,BODY_TYPES} from './player-profiles.js';
import {runnerPoint,leadPosition} from './runner-motion.js';
import {TIMING} from './presentation-timing.js';
import {createBattedFlight,sampleBattedFlight} from './batted-flight.js';
import {contactFeedback} from './contact-feedback.js';
import {planRunner,runnerProgress} from './running-timing.js';
import {locomotionPose,capturePose,blendPose,smoothStep} from './motion-blend.js';
import {QUALITY,initialQuality,createQualityController} from './render-quality.js';
import {createImpactMotion} from './impact-motion.js';
...
const quality=createQualityController(initialQuality({mobile:host.clientWidth<=760,cores:navigator.hardwareConcurrency||8,memory:navigator.deviceMemory||8}));
let renderPreset=QUALITY[quality.level],hitFeedback=null;
const impactMotion=createImpactMotion(reduced);let impactFrame=impactMotion.sample(0);
function sensation(kind,impact){if(model.effects===false)return;impactMotion.trigger(kind);onStatus(null,{sensation:{kind,impact}});}
let renderer;
renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'});
renderer.setPixelRatio(Math.min(devicePixelRatio,renderPreset.pixelRatio));
renderer.setSize(Math.max(1,host.clientWidth),Math.max(1,host.clientHeight));renderer.shadowMap.enabled=Boolean(renderPreset.shadow);renderer.shadowMap.type=THREE.PCFSoftShadowMap;
...
const sun=new THREE.DirectionalLight('#fff0c3',3.2);sun.position.set(-28,45,22);sun.castShadow=true;sun.shadow.mapSize.set(renderPreset.shadow||512,renderPreset.shadow||512);sun.shadow.camera.left=-38;sun.shadow.camera.right=38;sun.shadow.camera.top=28;sun.shadow.camera.bottom=-45;sun.shadow.camera.far=120;sun.shadow.normalBias=.04;sun.target.position.set(0,0,-15);scene.add(sun,sun.target);
const materials={};
...
// Running has its own scene-space rig: never unmirror a live batting pose.
const batterRunner=player('#eee7d4',0,0,'09','runner');batterRunner.root.visible=false;
let pitchSign=1,batSign=1;
let pitchForm=PITCH_FORMS[0],batForm=BAT_FORMS[0],playerIds={};
let teamColors=['#ece1bb','#9d4336'],teamSeed='';
function configurePlayers(next={}){const attack=next.attacker?next.attacker==='p2':next.attackTeam||0;const batterId=next.batterId||playerIds.batterId||('p'+(attack?2:1)+':batter:0'),pitcherId=next.pitcherId||playerIds.pitcherId||('p'+(attack?1:2)+':pitcher:0');playerIds={batterId,pitcherId};const p=playerProfile(teamSeed,pitcherId,'pitcher'),b=playerProfile(teamSeed,batterId,'batter');pitchForm=p;batForm=b;pitcher.setBuild(p.body);batter.setBuild(b.body);batterRunner.setBuild(b.body);pitchSign=p.hand==='L'?-1:1;batSign=b.hand==='L'?-1:1;pitcher.root.scale.x=pitchSign;pitcher.badge.scale.x=pitchSign;batter.badge.scale.x=batSign;battingRig.scale.x=batSign;}
...
function uniformTeams(attack){const a=teamColors[attack?1:0],d=teamColors[attack?0:1];batter.setUniform(a);batterRunner.setUniform(a);runners.forEach(r=>r.setUniform(a));pitcher.setUniform(d);catcher.setUniform(d);defense.setUniform(d);}
...
function applyQuality(level){renderPreset=QUALITY[level];renderer.setPixelRatio(Math.min(devicePixelRatio,renderPreset.pixelRatio));renderer.shadowMap.enabled=Boolean(renderPreset.shadow);if(sun.shadow.mapSize.x!==(renderPreset.shadow||512)){sun.shadow.map?.dispose();sun.shadow.map=null;sun.shadow.mapSize.set(renderPreset.shadow||512,renderPreset.shadow||512);}scene.traverse(o=>{if(o.material)for(const m of Array.isArray(o.material)?o.material:[o.material])m.needsUpdate=true;});stadium.setQuality(renderPreset.crowd);trailGeometry.setDrawRange(0,renderPreset.trail);onStatus(null,{quality:{mode:quality.mode,level}});}
function setQuality(mode){applyQuality(quality.set(mode));}
...
function animatePitcher(t){
 const w=state.phase==='windup'?clamp(t/(TIMING.windup*pitchForm.tempo),0,1):state.phase==='flight'?1+state.flight/.7:state.phase==='catch'?1+(state.duration+state.catchAge)/.7:state.phase==='result'?1+(state.duration+(state.contact?0:TIMING.catch))/.7+state.resultAge:0;
 ...
 pitcher.hip.position.set(-.025*lift,.80-.10*drive+.10*recover,.29*drive*(1-recover));pitcher.hip.rotation.set(0,-.2*lift+.22*drive*(1-recover),0);pitcher.torso.rotation.set(.05+.25*drive*(1-recover),-pitchForm.coil*lift+.22*drive*(1-recover),-(.04+(pitchForm.lean||0))*drive);
 plantFoot(pitcher,0,new THREE.Vector3(.13-.03*drive,.085+pitchForm.lift*lift,.05+pitchForm.stride*drive*(1-recover)));
 plantFoot(pitcher,1,new THREE.Vector3(-.13,.085+.12*finish*(1-recover),-.04-.18*finish*(1-recover)));
 const ready=new THREE.Vector3(-.01,1.26,.24),cocked=new THREE.Vector3(pitchForm.cockX??-.47,pitchForm.cockY??1.68,-.13),released=new THREE.Vector3(pitchForm.reach,pitchForm.slot,.72),follow=new THREE.Vector3(.17,.98,.58);
...
function animateBatter(dt){
 state.swingAge+=dt;const age=state.swingAge,live=age<.95,contactAt=.16;
 const ease=v=>{v=clamp(v,0,1);return v*v*(3-2*v)};
 const turn=live?ease(age/.28)*(1-ease((age-.57)/.38)):0,load=state.phase==='windup'?Math.sin(state.elapsed/(TIMING.windup*pitchForm.tempo)*Math.PI)*batForm.load:0;
 battingRig.scale.x=batSign;batter.root.position.set(-.82,0,.05);batter.root.rotation.y=Math.PI/2;
 const y=Number.isFinite(target.y)&&state.phase!=='ready'?target.y:1.05;
 const extension=live?ease(age/contactAt)*(1-ease((age-contactAt)/.36)):0;
 const heightCrouch=Math.max(0,batForm.body?.torsoExtra||0)*clamp((1.15-y)/.55,0,1)*extension;
 const crouch=Math.max(0,1-y)*.5*extension+heightCrouch+batForm.crouch*(1-extension);
 batter.hip.position.set(0,.77-crouch,.025*turn+.075*extension);batter.hip.rotation.y=turn*.38+(batForm.open||0)*(1-extension);batter.torso.rotation.set(.08+crouch*.4,-load+turn*.60,0);
 batter.head.rotation.set(.025,state.phase==='flight'?lerp(1.1,.12,clamp(state.flight/state.duration,0,1)):live?.12:1.1,0);
 const legLift=state.phase==='windup'?Math.sin(clamp(state.elapsed/(TIMING.windup*pitchForm.tempo),0,1)*Math.PI)*(batForm.kick||0):0;
 plantFoot(batter,0,new THREE.Vector3(batForm.width,.08+legLift,.035+(batForm.open||0)*(1-extension)));plantFoot(batter,1,new THREE.Vector3(-batForm.width+.01,.08+turn*.04,.015));
 const impact=choreography?.impact||{barrel:.65,tilt:0};
 const grip=new THREE.Vector3(-.62,batForm.gripY,batForm.gripZ),direction=new THREE.Vector3(-.18+(live?0:Math.sin(state.elapsed*2.7)*batForm.waggle),.91,.36);
 if(live){
  const u=clamp(age/contactAt,0,1),v=ease((age-contactAt)/.36),recover=ease((age-.57)/.38);
  grip.lerp(new THREE.Vector3(target.x/batSign-impact.barrel*Math.cos(impact.tilt),y-impact.barrel*Math.sin(impact.tilt),0),u).lerp(new THREE.Vector3(-.72,1.40,-.15),v).lerp(new THREE.Vector3(-.62,batForm.gripY,batForm.gripZ),recover);
  const angle=lerp(1.75,0,u)-v*2;direction.set(Math.cos(angle)*Math.cos(impact.tilt),lerp(.7,Math.sin(impact.tilt),u)+v*.7,Math.sin(angle)).lerp(new THREE.Vector3(-.18,.91,.36),recover);
 }
 batPivot.position.copy(grip);batPivot.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),direction.normalize());
 battingRig.updateWorldMatrix(true,true);
 reach(batter.left,battingRig.localToWorld(grip.clone()));reach(batter.right,battingRig.localToWorld(grip.clone().addScaledVector(direction,.065)));
}

let runRoute=[],runDuration=0,runnerPlans=[],choreography=null,battedFlight=null,callEvents=[],callIndex=0,revealAt=0,fieldShot='',cameraFitPoints=[];
...
function animateRunner(t){for(const r of runnerPlans){
 const p=r.from===0?batterRunner:runners[r.from-1];
 if(t<r.delay){if(r.from>0){p.root.visible=true;const lead=r.tagUp?Math.max(0,1-t/Math.max(.1,choreography.catchAt-.1))*1.15:1.15;p.root.position.fromArray(leadPosition(r.from,lead));runPose(p,t,r.tagUp);p.head.rotation.y=.4;}continue;}
 if(r.from===0&&!r.takeoff){r.takeoff=batter.bodyMesh.skeleton.bones.map(b=>({position:b.position.clone(),rotation:b.quaternion.clone()}));r.start=batter.root.getWorldPosition(new THREE.Vector3());p.root.scale.x=batSign;p.badge.scale.x=batSign;}
 const stopped=Number.isFinite(r.stopAt)&&t>=r.stopAt,elapsed=Math.max(0,Math.min(t,r.stopAt??Infinity)-r.delay),distance=r.retreat||r.returnAfterForce?1:Math.max(0,r.to-r.from),duration=r.duration,progress=runnerProgress(elapsed,duration),walk=r.walk;
 p.root.visible=!(r.out&&Number.isFinite(r.outAt)&&t>=r.outAt+.18)&&!(progress>=1&&((r.out&&r.outAt==null)||(!r.out&&r.to===4)));if(!p.root.visible)continue;
 if(r.retreat||r.returnAfterForce){const v=Math.sin(progress*Math.PI)*.14,from=r.start||new THREE.Vector3(...BASES[r.from]),to=new THREE.Vector3(...BASES[Math.min(4,r.attemptTo||r.from+1)]);p.root.position.lerpVectors(from,to,v);if(r.from>0)p.root.position.add(new THREE.Vector3(...leadPosition(r.from)).sub(from).multiplyScalar(1-progress));const direction=to.clone().sub(from).multiplyScalar(progress<.5?1:-1);p.root.rotation.y=Math.atan2(direction.x,direction.z);}
 else if(distance){const sample=runnerPoint(r.from,r.to,progress,r.start?.toArray()||(r.tagUp?BASES[r.from]:r.from?leadPosition(r.from):BASES[0]));p.root.position.fromArray(sample.point);p.root.rotation.y=sample.yaw;
  if(r.runThrough&&progress>=1){const u=clamp((elapsed-duration)/.65,0,1);p.root.position.add(new THREE.Vector3(1,0,-1).normalize().multiplyScalar(3.2*Math.sin(u*Math.PI/2)));}
 }else p.root.position.fromArray(BASES[r.from]);
 const moving=(progress<1||r.runThrough&&elapsed<duration+.65)&&distance>0&&!stopped;
 const delta=r.lastPosition?p.root.position.distanceTo(r.lastPosition):0;r.travel=(r.travel||0)+delta;const speed=moving?delta/Math.max(.01,t-(r.lastTime??t-.025)):0;
 locomotionPose(p,r.travel,speed);r.lastTime=t;r.lastPosition=p.root.position.clone();
 if(r.slide&&progress>.86&&!stopped){const running=capturePose(p);runPose(p,t,false,true);const settle=1-smoothStep((elapsed-duration-.12)/.4);blendPose(p,running,smoothStep((progress-.86)/.12)*settle);}
 if(r.takeoff&&elapsed<.3){const blend=clamp(elapsed/.3,0,1),u=blend*blend*(3-2*blend),startYaw=batSign*Math.PI/2,delta=p.root.rotation.y-startYaw;p.root.rotation.y=startYaw+Math.atan2(Math.sin(delta),Math.cos(delta))*u;p.bodyMesh.skeleton.bones.forEach((b,i)=>{b.position.lerpVectors(r.takeoff[i].position,b.position,u);b.quaternion.slerpQuaternions(r.takeoff[i].rotation,b.quaternion,u)});}
}}
function settleRunnerBuilds(){
 if(!choreography||(!choreography.contact&&!choreography.walk)||choreography.foul)return;
 const next=[null,null,null];
 if(!choreography.inningEnded)for(const r of runnerPlans)if(!r.out&&!r.stopped&&r.to>=1&&r.to<=3)next[r.to-1]=r.from===0?batter.build:runners[r.from-1].build;
 runners.forEach((p,i)=>p.setBuild(next[i]||BODY_TYPES[0]));
}
function startRunners(){batterRunner.root.visible=false;const throws=defense.throwTimes;
 runnerPlans=choreography.runs.map(r=>planRunner(choreography,r,throws));
 runRoute=runnerPlans.some(r=>r.from===0&&r.to>0)?BASES.slice(1,(runnerPlans.find(r=>r.from===0)?.to||0)+1):[];
 runDuration=Math.max(0,...runnerPlans.map(r=>r.to===r.from&&!r.retreat?0:Math.min(r.stopAt??Infinity,r.outAt!=null?r.outAt+.18:Infinity,r.delay+r.duration+(r.runThrough?.65:0))));
 runners.forEach((p,i)=>{p.root.visible=Boolean(currentPlay.basesBefore?.[i]);p.root.position.fromArray(leadPosition(i+1))});
}

const state={phase:'ready',elapsed:0,flight:0,duration:.65,swingAge:10,resultAge:0,contact:false,outcome:'',visualKey:'',plays:0};
let active=true,disposed=false,view='batter',model={},pendingModel=null,currentPlay=null,manualCamera=false,frameId=0;
...
function sync(next){const previousRole=model.defending,previousPitch=pitchSign,previousBat=batSign;
 // The server has already advanced the game. Keep its next-half state queued
 // until this pitch and every fielding/runner animation have actually finished.
 if(next?.pending||state.phase!=='ready'){pendingModel=next||{};model={...model,locked:true,shop:next?.shop??model.shop,effects:next?.effects??model.effects};}
 else{model=next||{};pendingModel=null;}if(state.phase==='ready'&&!model.pending)configurePlayers(model);if(state.phase==='ready'&&!manualCamera&&(previousRole!==model.defending||previousPitch!==pitchSign||previousBat!==batSign||view==='zone'))setCamera(model.defending?'pitcher':'batter',false,true);if(state.phase==='ready')uniformTeams(model.attackTeam||0);if(!model.pending&&state.phase==='ready')runners.forEach((runner,i)=>{runner.root.visible=Boolean(model.bases?.[i]);runner.root.position.fromArray(leadPosition(i+1));runPose(runner,0,false)});targetMarker.visible=Number.isInteger(model.pick)&&state.phase!=='flight';if(targetMarker.visible)targetMarker.position.copy(cellPoint(model.pick));
...
function play(resolved){if(!resolved||resolved.key===state.visualKey)return;state.visualKey=resolved.key;state.plays++;followSuppressed=false;following=false;hoverCell=null;resetCatcher();currentPlay={basesBefore:[...(model.bases||[])],...resolved};model={...model,defending:resolved.defending??model.defending,attackTeam:resolved.attacker?Number(resolved.attacker==='p2'):model.attackTeam,bases:[...currentPlay.basesBefore]};configurePlayers({...model,...resolved});choreography=planPlay({...currentPlay,key:resolved.variationKey||resolved.key,batHand:batSign===1?'R':'L'});currentPlay.outcome=choreography.out;uniformTeams(resolved.attacker?resolved.attacker==='p2':model.attackTeam||0);runnerPlans=[];runRoute=[];runDuration=0;batterRunner.root.visible=false;defense.reset();autoPlayCamera=!manualCamera||view==='zone';if(autoPlayCamera)setCamera((resolved.defending??model.defending)?'pitcher':'batter',false,true);state.phase='windup';impactMotion.clear();state.revealed=false;hitFeedback=null;callEvents=[];callIndex=0;state.elapsed=0;state.flight=0;state.swingAge=10;state.contact=false;state.resultAge=0;state.outcome=choreography.out;state.duration=clamp(18.44/((Number(resolved.speed)||140)/3.6),.38,.9);target.copy(cellPoint(Number.isInteger(resolved.actualCell)?clamp(resolved.actualCell,0,24):12));
...
function resolveVisual(){state.phase='result';state.resultAge=0;const out=state.outcome,text=currentPlay.text||'';state.contact=isContact();
 if(state.contact){velocity.set(Math.sin(choreography.angle)*choreography.distance/choreography.catchAt,0,-Math.cos(choreography.angle)*choreography.distance/choreography.catchAt);battedStart.copy(ball.position);battedFlight=createBattedFlight(choreography,battedStart.toArray(),[choreography.end[0],.085,choreography.end[2]]);trail.visible=!reduced;contactRing.position.copy(ball.position);contactRing.material.color.set({classic:'#f4edd3',night:'#ff9950',retro:'#d3ad73',neon:'#8cddff'}[model.shop?.equippedEffect]||'#f4edd3');contactRing.material.opacity=.85;defense.begin({...currentPlay,choreography},battedStart);hitFeedback=contactFeedback(choreography,defense.flight||battedFlight);onStatus(null,{impact:choreography.impact});sensation(choreography.impact.kind==='barrel'?'contact':'soft',choreography.impact);}
 startRunners();
 ...
 scheduleCalls();
 if(!state.contact)revealResult();
}
function revealResult(){if(state.revealed)return;state.revealed=true;
 const words=String(currentPlay.playText||currentPlay.text||'');
 const accent=/삼진/.test(words)?'strikeout':choreography.home?'homerun':choreography.fly||choreography.ground&&!choreography.hit?'out':!state.contact&&state.outcome==='swinging_strike'?'strike':null;
 if(accent)sensation(accent);
 const description=describePlay(currentPlay);pitchHistory.unshift({...lastLocation,point:target.toArray(),batCell:currentPlay.batCell,pitch:description.pitch,result:description.title});pitchHistory.splice(5);refreshHistoryMarkers();
 lastCommentary=description;onStatus(null,{history:[...pitchHistory],reveal:true,commentary:lastCommentary,feedback:hitFeedback});
}
function scheduleCalls(){
 const shot=choreography,throws=defense.throwTimes;
 callEvents=[];callIndex=0;
 if(!shot.contact){revealAt=0;return;}
 if(shot.fly)callEvents.push({at:shot.catchAt,title:'잡았습니다! 아웃!'});
 if(!shot.fly)for(const move of runnerPlans.filter(r=>r.out)){const at=throws.find(l=>l.base===move.to)?.time;if(at!=null)callEvents.push({at,title:(move.to===4?'홈':move.to+'루')+'에서 아웃!'});}
 if(shot.home){let lo=0,hi=shot.catchAt;for(let i=0;i<24;i++){const mid=(lo+hi)/2,point=sampleBattedFlight(battedFlight,mid);if(Math.hypot(point[0],point[2])<100)lo=mid;else hi=mid;}callEvents.push({at:hi,title:'담장을 넘겼습니다! 홈런!'});}
 if(shot.foul)callEvents.push({at:Math.min(.65,shot.catchAt*.4),title:'파울!'});
 const arrivals=runnerPlans.filter(r=>!r.out&&!r.stopped&&r.to>r.from).map(r=>({at:r.delay+r.duration,title:r.to===4?'홈인!':r.to+'루 세이프!'}));
 if(!shot.home)callEvents.push(...arrivals);
 const decisive=shot.foul?callEvents[0].at:Math.max(shot.fly?shot.catchAt:shot.ground&&!shot.hit?(throws.at(-1)?.time||shot.catchAt):shot.home?(callEvents[0]?.at||shot.catchAt):0,...runnerPlans.filter(r=>r.out&&r.outAt!=null).map(r=>r.outAt),...arrivals.map(e=>e.at));
 revealAt=decisive;callEvents=callEvents.filter(e=>e.at<decisive-.1).sort((a,b)=>a.at-b.at);
}
function updateCalls(t){while(callIndex<callEvents.length&&t>=callEvents[callIndex].at){const event=callEvents[callIndex++];onStatus(null,{commentary:{title:event.title,detail:'',pitch:''}});}if(!state.revealed&&t>=revealAt)revealResult();}
...
function update(dt){state.elapsed+=dt;if(!(state.phase==='result'&&defense.active))animatePitcher(state.elapsed);animateBatter(dt);
 if(state.phase==='windup'){scene.updateMatrixWorld(true);pitcher.right.hand.getWorldPosition(ball.position);if(state.elapsed>=TIMING.windup*pitchForm.tempo){release.copy(ball.position);state.phase='flight';state.flight=0;trailAt(ball.position,true);trail.visible=!reduced;}}
 ...
 const take=['ball','walk'].includes(state.outcome)||currentPlay.strikeStyle==='looking'||/볼넷|루킹/.test(currentPlay.text||'');if(!take&&state.flight>=state.duration-.16&&state.swingAge>1)state.swingAge=Math.max(0,state.flight-(state.duration-.16));if(t>=1){if(isContact()){state.swingAge=.16;animateBatter(0);resolveVisual();}else{state.phase='catch';state.catchAge=0;catcher.root.position.x=target.x*.65;const mitt=new THREE.Vector3(target.x,clamp(target.y,.4,1.4),.98);reach(catcher.left,mitt);scene.updateMatrixWorld(true);catcherGlove.getWorldPosition(gloveTarget);}}}
 ...
 else if(state.phase==='result'){state.resultAge+=dt;const t=state.resultAge;animateRunner(t);contactRing.material.opacity=Math.max(0,.85-t*3);contactRing.scale.setScalar(1+t*3);if(state.contact&&defense.active){defense.update(t,ball,runnerPlans.map(r=>({...r,position:(r.from===0?batterRunner:runners[r.from-1]).root.position.toArray()})));trail.visible=!reduced&&(defense.stage==='throw'||defense.stage==='chase');trailAt(ball.position);}else if(state.contact){const u=clamp(t/choreography.catchAt,0,1);ball.position.fromArray(sampleBattedFlight(battedFlight,t));ball.rotation.x+=dt*22;trailAt(ball.position);if(u===1)trail.visible=false;}else{ball.position.copy(gloveTarget);ball.visible=t<.45;}updateCalls(t);const end=defense.active?defense.duration:state.contact?choreography.catchAt+TIMING.settle:/삼진/.test(currentPlay.playText||currentPlay.text||'')?1.1:TIMING.callHold;if(t>Math.max(end,runDuration+.35,revealAt+.85)){settleRunnerBuilds();runnerPlans=[];runRoute=[];batterRunner.root.visible=false;defense.settle();resetCatcher();state.phase='ready';ball.visible=false;trail.visible=false;following=false;const nextModel=pendingModel||model;sync({...nextModel,pending:false});if(!followSuppressed||autoPlayCamera)setCamera(model.defending?'pitcher':'batter',false,true);onStatus(null,{complete:true});}}
 else{ball.visible=false;}

 const call=state.phase==='result'&&!state.contact&&!['ball','walk'].includes(state.outcome),strikeout=/삼진/.test(currentPlay?.playText||currentPlay?.text||'');umpire.hip.position.y=call?.9:.62;umpire.torso.rotation.x=call?0:.25;umpire.legs.forEach(l=>{l.pivot.rotation.x=-.5;l.knee.rotation.x=.8});umpire.right.pivot.rotation.set(call?-.9-Math.sin(Math.min(state.resultAge,1)*Math.PI)*.7:-.5,0,call?-.85:0);umpire.right.elbow.rotation.x=call?(strikeout?-.2:-1.6):-.5;umpire.torso.rotation.y=call&&strikeout?Math.sin(Math.min(state.resultAge,1)*Math.PI)*.55:0;umpire.root.visible=!['zone','batter'].includes(view);
 ...
 if(following){const direction=new THREE.Vector3(velocity.x,0,velocity.z).normalize();const chase=ball.position.clone().addScaledVector(direction,defense.active?-10:-6);chase.x+=defense.active?7:0;chase.y=Math.max(defense.active?7:2.4,ball.position.y+(defense.active?5:2.5));const fieldFrame=defense.active?defense.cameraFrame(ball.position,camera.aspect,runnerPlans.filter(r=>!r.out||state.resultAge<(r.outAt??Infinity)).map(r=>({...r,position:(r.from===0?batterRunner:runners[r.from-1]).root.position.toArray()}))):null;fieldShot=fieldFrame?.shot||'ball-flight';cameraFitPoints=fieldFrame?.points||[ball.position.toArray()];if(!wasFollowing){camera.fov=fieldFrame?.fov||50;camera.updateProjectionMatrix();camera.position.copy(fieldFrame?.position||chase);lookCurrent.copy(fieldFrame?.target||ball.position);onStatus(null,{view:'field'})}else{camera.fov=lerp(camera.fov,fieldFrame?.fov||50,1-Math.exp(-dt*5));camera.updateProjectionMatrix();camera.position.lerp(fieldFrame?.position||chase,1-Math.exp(-dt*4));lookCurrent.lerp(fieldFrame?.target||ball.position,1-Math.exp(-dt*4));}onStatus(null,{following:true,fielding:defense.label});}
 else{camera.position.lerp(cameraGoal,smooth);lookCurrent.lerp(lookGoal,smooth);}
 camera.lookAt(lookCurrent);if(model.effects===false)impactMotion.clear();impactFrame=impactMotion.sample(dt);camera.rotateY(impactFrame.x);camera.rotateX(impactFrame.y);camera.rotateZ(impactFrame.roll);camera.zoom=1+impactFrame.zoom;camera.updateProjectionMatrix();camera.updateMatrixWorld();const batterRun=runnerPlans.find(r=>r.from===0);const hasTakenOff=state.phase==='result'&&batterRun&&state.resultAge>=batterRun.delay;batter.root.visible=(view!=='zone'||state.phase!=='ready')&&!hasTakenOff;batPivot.visible=batter.root.visible;catcher.root.visible=following||!['batter','zone'].includes(view);visibleMitt.visible=!catcher.root.visible&&(state.phase==='catch'||(state.phase==='result'&&!state.contact));if(visibleMitt.visible){catcherGlove.getWorldPosition(visibleMitt.position);catcherGlove.getWorldQuaternion(visibleMitt.quaternion);}zone.visible=!following;historyMarkers.visible=zone.visible;targetMarker.visible=zone.visible&&Number.isInteger(model.pick)&&state.phase==='ready';locationMarker.visible=false;paintZone();
}
...
let previous=performance.now();function frame(now){if(disposed)return;const dt=clamp((now-previous)/1000,0,.05);previous=now;if(active&&host.clientWidth&&host.clientHeight&&!document.hidden){const nextQuality=quality.sample(dt*1000,state.phase==='ready');if(nextQuality)applyQuality(nextQuality);update(dt);renderer.render(scene,camera)}frameId=requestAnimationFrame(frame)}frameId=requestAnimationFrame(frame);
return {setQuality,restoreHistory(records){if(state.phase!=='ready'||!Array.isArray(records))return;pitchHistory.length=0;for(const [i,p] of records.slice(0,5).entries()){if(!Number.isInteger(p.actualCell))continue;const point=cellPoint(p.actualCell);const outside=p.isBall||['wild','bait'].includes(p.execution);if(outside)point.x+=.52;pitchHistory.push({key:'restored:'+i,cell:p.actualCell,batCell:p.batCell,point:point.toArray(),outside,location:pitchLocation(p)})}lastLocation=pitchHistory[0]||null;refreshHistoryMarkers();},sync,play,setRoomSeed(seed){seedTeams(seed);configurePlayers(model);defense.setHands(teamSeed);if(state.phase==='ready'){uniformTeams(model.attackTeam||0);setCamera(view,manualCamera,true)}},reset(){runners.forEach(p=>p.setBuild(BODY_TYPES[0]));impactMotion.clear();impactFrame=impactMotion.sample(0);camera.zoom=1;camera.updateProjectionMatrix();state.phase='ready';state.visualKey='';state.plays=0;currentPlay=null;pendingModel=null;hitFeedback=null;lastLocation=null;lastCommentary=null;runRoute=[];runDuration=0;following=false;state.swingAge=10;pitchHistory.length=0;runnerPlans=[];batterRunner.root.visible=false;refreshHistoryMarkers();seedTeams(crypto.getRandomValues(new Uint32Array(1))[0]);playerIds={};configurePlayers(model);defense.setHands(teamSeed);defense.reset();setCamera(model.defending?'pitcher':'batter',false,true);onStatus(null,{history:[],commentary:{title:'어떤 코스로 승부할까요?',detail:'가상 스트라이크존에서 코스를 선택하세요.',pitch:''}})},setCamera,setFollow(value){followEnabled=Boolean(value);if(!followEnabled&&following)setCamera(model.defending?'pitcher':'batter',false,true)},setActive(value){const next=Boolean(value);if(active!==next){active=next;previous=performance.now()}},snapshot(){const point=o=>o.getWorldPosition(new THREE.Vector3()).toArray();const rig=p=>({head:point(p.head),feet:p.legs.map(l=>point(l.foot)),hands:[point(p.left.hand),point(p.right.hand)]});return{builds:{pitcher:pitcher.build,batter:batter.build,runner:batterRunner.build,bases:runners.map(p=>p.build)},impactMotion:{...impactFrame},quality:{mode:quality.mode,level:quality.level,pixelRatio:renderer.getPixelRatio(),shadows:renderer.shadowMap.enabled,trail:renderPreset.trail},feedback:state.revealed?hitFeedback:null,impact:state.contact?choreography?.impact:null,players:{pitcher:rig(pitcher),batter:rig(batter),catcher:rig(catcher)},phase:state.phase,revealed:Boolean(state.revealed),revealAt,resultAge:state.resultAge,fieldShot,cameraFitPoints,framing:cameraFitPoints.map(p=>new THREE.Vector3(...p).project(camera).toArray()),trajectory:choreography?{kind:choreography.kind,distance:choreography.distance,end:[...choreography.end],contact:choreography.contact,profile:choreography.flight.type,exitSpeed:choreography.flight.exitSpeed}:null,teamColors:[...teamColors],profiles:{pitcher:{...pitchForm},batter:{...batForm}},playerIds:{...playerIds},uniforms:{batter:batter.uniformColor,pitcher:pitcher.uniformColor},attackTeam:model.attackTeam,runnerPlans:runnerPlans.map(({takeoff,start,...r})=>({...r,position:(r.from===0?batterRunner:runners[r.from-1]).root.position.toArray()})),historyMarkerCount:historyMarkers.children.filter(o=>o.isSprite).length,lastBatCell:pitchHistory[0]?.batCell,umpireCall:state.phase==='result'&&!state.contact&&!['ball','walk'].includes(state.outcome),key:state.visualKey,plays:state.plays,outcome:state.outcome,actualCell:currentPlay?.actualCell,ball:ball.position.toArray(),bases:[...(model.bases||[])],pick:model.pick,moundDistance:18.44,camera:view,following,cameraFov:camera.fov,modelRig:'skinned-baseball-v1',pitcherYaw:pitcher.root.rotation.y,battingMirror:battingRig.scale.x,batterVisible:batter.root.visible,runnerVisible:batterRunner.root.visible,runnerMirror:batterRunner.root.scale.x,runnerPosition:batterRunner.root.position.toArray(),batGripWorld:battingRig.localToWorld(batPivot.position.clone()).toArray(),batTopHandWorld:battingRig.localToWorld(batPivot.position.clone().add(new THREE.Vector3(0,.065,0).applyQuaternion(batPivot.quaternion))).toArray(),stadium:{...stadium.stats},timing:{...TIMING},renderStats:{calls:renderer.info.render.calls,triangles:renderer.info.render.triangles},cameraPosition:camera.position.toArray(),cameraTarget:lookCurrent.toArray(),commentary:lastCommentary,history:[...pitchHistory],location:lastLocation,rangeCells:[...(model.rangeCells||[])],rangeWeights:{...model.rangeWeights},catcherHand:'R',anatomy:{rightShoulderX:catcher.right.pivot.position.x,leftShoulderX:catcher.left.pivot.position.x},handedness:{pitch:pitchSign===1?'R':'L',bat:batSign===1?'R':'L'},bodyYaw:batter.root.rotation.y,batterPosition:batterRunner.root.visible?batterRunner.root.position.toArray():point(batter.root),running:state.phase==='result'&&runRoute.length>0&&batterRunner.root.visible,runBases:runRoute.length,contact:state.contact,effectOpacity:contactRing.material.opacity,glove:gloveTarget.toArray(),swingAge:state.swingAge,batPosition:batPivot.position.toArray(),batVisible:batPivot.visible,defense:defense.snapshot(),zoneCells:zoneCells.map(tile=>{const p=tile.getWorldPosition(new THREE.Vector3()).project(camera);return{cell:tile.userData.cell,x:(p.x+1)/2,y:(1-p.y)/2,visible:zone.visible&&p.z>-1&&p.z<1}})}},dispose(){disposed=true;cancelAnimationFrame(frameId);observer.disconnect();for(const [type,fn] of [['pointerdown',down],['pointerup',up],['pointermove',move],['pointerleave',leave],['pointercancel',leave],['keydown',keydown]])renderer.domElement.removeEventListener(type,fn);scene.traverse(object=>{object.geometry?.dispose();object.skeleton?.dispose();if(object.material){for(const material of Array.isArray(object.material)?object.material:[object.material]){material.map?.dispose();material.dispose()}}});renderer.dispose();renderer.domElement.remove()}};
}


export {playImpactSound} from './contact-feedback.js';
