...
try{
 ...
 for(const entry of ['/public/game/index.html','/static/index.html','/static/game/index.html']){
  ...
  await page.reload();await page.evaluate(()=>{requestMatch=async()=>{throw Error('offline fixture')};start('solo')});await page.waitForFunction(()=>state.mode==='solo'&&!state.net);await page.locator('.game .demo3DEntry').click();await page.waitForFunction(()=>pitchit3D.snapshot().scene);assert.equal(await page.locator('[data-quality3d]').inputValue(),'low');
 }
 assert.deepEqual(errors,[]);console.log('PASS: '+physical+' contact profiles; '+checked+' base-play timelines at all quality levels; force touches, glove possession/tag, safe races, backup roles; result-only feedback, persistent mobile quality and three full game entries.');
